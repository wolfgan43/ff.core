...
class Storage extends vgCommon
{
    ...
    public function cmd($action, $what, $table)
    {
        $this->clearResult();
    ...
    {
        if(is_array($this->$param) && array_key_exists($source, $this->$param)) {
                    ...
                    if(is_array($query)) {
                        switch($this->action)
                        {
                            case "read":
                                ...
                                if($this->use_cache && !$this->exts) {
                                    if(isset(Storage::$cache[$cache_key]["data"])) {
                                        if($this::DEBUG)                { Debug::dumpLog("query_duplicate", $query); }

                                        $this->result[$service]         = Storage::$cache[$cache_key]["data"];
                                        return $service;
                                    }
                                }
                                ...
                                if($res) {
                                    ...
                                    if($count < $this::MAX_NUMROWS) {
                                        ...
                                        if($this->rawdata || $count > $this::MAX_RESULTS) {
                            ...
                            case "delete":
                                ...
                                break;
                            default:
                                $res                                = $db->cmd($query, $this->action);
                                if($res !== null) {
                                    $this->result                   = $res;
                                } else {
                                    $this->isError("Sql: unable to execute command" . ($this::DEBUG ? ": " . print_r($query, true) : ""));
                                }
                        }
                    } else {
                        ...
                        switch($this->action)
                        {
                            ...
                            default:
                                ...
                                if($res) {
                                    ...
                                    $this->isError("noSql: unable to execute command" . ($this::DEBUG ? ": " . print_r($query, true) : ""));
                                }
                        }
                    } else {
                        $this->isError("Query: " . $query);
        ...
        switch($method) {
			case "IMAGE":
				if($source === true) {
				    ...
				    $frameworkCSS = new frameworkCSS();
					$res                                                    = $frameworkCSS->get($source, "icon-tag");
				}
				break;
			case "TIMEELAPSED":
				...
				if($time < 2 * $day) {
					if($time < $min ) {
						$res                                                = ffTranslator::get_word_by_code("about") . " " . ffTranslator::get_word_by_code("a") . " " . ffTranslator::get_word_by_code("minute") . " " . ffTranslator::get_word_by_code("ago");
					} else if($time > $day ) {
						$res                                                = ffTranslator::get_word_by_code("yesterday") . " " . ffTranslator::get_word_by_code("at") . " " . date("G:i", $source);
					} else {
						$tokens                                             = array(
                                                                                31536000 	=> 'year',
                                                                                2592000 	=> 'month',
						...
						foreach ($tokens as $unit => $text) {
							...
							$res                                            = floor($time / $unit);
							$res                                            .= ' ' . ffTranslator::get_word_by_code($text . (($res > 1) ? 's' : '')) . " " . ffTranslator::get_word_by_code("ago");
							break;
						}
					}
					break;
			...
			case "DATETIME":
				...
				$conv                                                       = array(
                                                                                $prefix . "01/" => " " . ffTranslator::get_word_by_code("Januaunable to updatery") . " "
                                                                                , $prefix . "02/" => " " . ffTranslator::get_word_by_code("February") . " "
                                                                                , $prefix . "03/" => " " . ffTranslator::get_word_by_code("March") . " "
                                                                                , $prefix . "04/" => " " . ffTranslator::get_word_by_code("April") . " "
                                                                                , $prefix . "05/" => " " . ffTranslator::get_word_by_code("May") . " "
                                                                                , $prefix . "06/" => " " . ffTranslator::get_word_by_code("June") . " "
                                                                                , $prefix . "07/" => " " . ffTranslator::get_word_by_code("July") . " "
                                                                                , $prefix . "08/" => " " . ffTranslator::get_word_by_code("August") . " "
                                                                                , $prefix . "09/" => " " . ffTranslator::get_word_by_code("September") . " "
                                                                                , $prefix . "10/" => " " . ffTranslator::get_word_by_code("October") . " "
                                                                                , $prefix . "11/" => " " . ffTranslator::get_word_by_code("November") . " "
                                                                                , $prefix . "12/" => " " . ffTranslator::get_word_by_code("December") . " "
                                                                            );
				$res                                                        = str_replace(array_keys($conv), array_values($conv), $res);
				if($prefix)
					$res                                                    = str_replace("/", ", ", $res);
				$res                                                        .= " " . ffTranslator::get_word_by_code("at") . " " . ffTranslator::get_word_by_code("hours") . " " . $ffRes->getValue("Time", FF_LOCALE);
