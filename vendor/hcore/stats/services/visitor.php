...
//todo: da creare gli aggregati
    ...
    public function write_stats($user = null, $page = null) {
		...
		if($user["ID"]) {
            ...
            $query                          = $this->getVisitorStats($user, $page);

            if($query) {
                $storage                    = $this->getStorage();

                $res                        = $storage->read($query["where"]);
                if(is_array($res)) {
                    $update                 = $storage->update(
                                                $this->stats->normalize_fields($query["set"], array_intersect_key($res["result"][0], $query["set"]))
                                                , $query["where"]
                                            );
                } else {
                    $insert                 = $storage->insert($this->stats->normalize_fields($query["insert"]));
                }
            }
        }
