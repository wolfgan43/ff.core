...
class Google_Client
{
  ...
  public function getArrToken()
  {
    $token = json_decode($this->getAuth()->getAccessToken());
  ...
  {
    $token =  json_decode($this->getAuth()->refreshToken($refreshToken)->getAccessToken());
	
	$arrRefreshToken = array(
		"access_token" => $token->access_token
		, "expires_in" => $token->expires_in
		, "expires" => $token->created + $token->expires_in
	);
    
	return $arrRefreshToken;
  }
