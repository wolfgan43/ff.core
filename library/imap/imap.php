...
class imap {
     ...
     */
    public function __construct($mailbox, $username, $password, $encryption = null) {
        $enc = '';
    ...
    public function search($criteria, $withbody = true) {
        $emails = array();	
        if(is_array($criteria) && count($criteria)) {
        	foreach($criteria AS $type => $search) {
        		$uType = strtoupper($type);
        		if(is_array($search) && count($search)) {
        			foreach($search AS $search_subvalue) {
        				if($search_subvalue) {
        					$found = imap_search($this->imap, $uType . ' "'.$search_subvalue.'"');
        					if(is_array($found) && count($found)) {
        						$emails = array_flip($found);
        					}
						}
					}
        		} else {
        			if($search) {
        				$found = imap_search($this->imap, $uType . ' "'.$search.'"');
        				if(is_array($found) && count($found)) {
        					$emails = array_flip($found);
        				}
					}
	    ...
	    foreach($emails AS $id => $message) {
            $emails[$id]= $this->formatMessage($id, $withbody);
		}
     ...
     */
    public function deleteMessage($id, $purge = true) {
...
*/
     ...
     */
    private function toAddress($headerinfos, $toString = false) {
        $email = "";
        $name = "";
     ...
     */
    private function arrayToAddress($addresses, $toString = false) {
        $addressesAsString = array();
        foreach($addresses as $address) {
            $addressesAsString[] = $this->toAddress($address, $toString);
        }
