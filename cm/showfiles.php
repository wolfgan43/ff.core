...
if ($db->nextRecord())
{
    ...
    do
    {
        ...
        if (is_callable("mod_security_get_domain"))
        {
    ...
    else
    {
        ...
        else
        {
            if((substr($db_file, 0, strlen("http://")) == "http://")
                ...
                if ($remote_mtime !== $local_mtime || DISABLE_CACHE === true)
                {
...
if($filepath && $mode)
{
    ...
    if(is_array($imgParams[$mode])) {
        ...
        if(DISABLE_CACHE !== true && is_file($final_file))
        {
...
if (DISABLE_CACHE !== true && strlen($_SERVER["HTTP_IF_NONE_MATCH"]) && substr($_SERVER["HTTP_IF_NONE_MATCH"], 0, strlen($etag)) == $etag)
{
