...
$tpl->set_var("media_thumb_class", cm_getClassByFrameworkCss("corner-thumbnail", "util"));
...
if(is_array($arrParentPath) && count($arrParentPath)) {
    foreach($arrParentPath AS $arrParentPath_value) { 
        ...
        $str_menu_parent_path .= ($str_menu_parent_path ? '<ul class="list-group">' : '') . '<li class="list-group-item ' . cm_getClassByFrameworkCss(array("text-nowrap", "text-overflow"), "util") . '" title="' . $parent_title . '"><a href="javascript:void(0);"' . cm_getClassByFrameworkCss("folder-open", "icon-tag") . " " . $parent_title . '</a>';
    }
}
...
if(is_array($referer) && count($referer)) {
    $tpl->set_var("referer_container_class", cm_getClassByFrameworkCss("group", "list"));
    foreach ($referer AS $ref_key => $ref_value) {
        ...
        $tpl->set_var("referer_class", cm_getClassByFrameworkCss($arrItemClass, "list"));
