...
if (MOD_SEC_PROFILING_SKIPSYSTEM !== "*")
{
	...
	//$base_path = (string)$cm->router->getRuleById("restricted")->reverse;
	...
	{
	//	$path = substr($key, strlen($base_path));
		$path = $key;
		foreach ($value as $subkey => $subvalue)
		{
			...
			$record["label"] .= " " . $subvalue["label"];

			if ($subvalue["acl"])
			{
				$acl = "";
				foreach ($subvalue["acl"] as $acl_key => $acl_val)
				{
					if ($acl_val == 1)
						$acl .= "Utente";
					if ($acl_val == 2)
	...
	if (strlen(MOD_SEC_PROFILING_SKIPSYSTEM))
	{
		...
		//	$path = substr($key, strlen($base_path));
			...
			foreach ($value as $subkey => $subvalue)
			{
				if (in_array($subvalue["location"], $toskip))
				{
					foreach ($globals->recordset as $rec_key => $rec_val)
					{
						if (strpos($globals->recordset[$rec_key]["path"]->getValue(), $path) === 0)
							unset($globals->recordset[$rec_key]);
					}
					reset ($globals->recordset);
				}
			}
			reset($value);
		}
		...
		$globals->recordset = array_slice($globals->recordset, 0);
	}
}
...
$cm->oPage->tplAddJs("profiling", null, null, false, $cm->oPage->isXHR(), $script, false, "bottom");
//$cm->oPage->addContent($script);
