	...
	else 
	{
		...
		elseif (!MOD_SEC_CSRF_PROTECTION || (get_session("__FF_SESSION__", $session_data) == $_REQUEST[MOD_SEC_CSRF_PROTECTION_PARAM]))
		{
			if (!$only_check)
			{
				...
				if(defined("MOD_SEC_ENABLE_USER_TRACE") && constant("MOD_SEC_ENABLE_USER_TRACE") && $path !== null)
...
function mod_security_create_session($UserID = null, $UserNID = null, $Domain = "", $DomainID = "", $path = null, $disable_events = false, $lang_default = null, $cookiehash = null, $permanent_session = false)
{
	$cm = cm::getInstance();
...
function mod_security_set_user_by_social($social, $UserParams, $UserField, $UserToken = array(), $ID_domain = null, $disable_events = false, $skip_redirect = false, $path_info = null)
{
	...
	if ($db->nextRecord())
	{
	    if($db->getField("status", "Number", true) > 0) 
	    {
			...
			mod_security_create_session($userID, $userNID, null, $ID_domain, null, $disable_events || $skip_redirect );
			if(!$disable_events) 
			{
