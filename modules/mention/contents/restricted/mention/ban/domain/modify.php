...
if(isset($_REQUEST["frmAction"]) && isset($_REQUEST["setstatus"])) {
    $sSQL = "UPDATE " . CM_TABLE_PREFIX . "mod_mention_ban_domain
                    SET status = " . $db->toSql($_REQUEST["setstatus"]) . "
                    WHERE 
                    	" . CM_TABLE_PREFIX . "mod_mention_ban_domain.ID = " . $db->toSql($_REQUEST["keys"]["ID"], "Number");
    $db->execute($sSQL);
...
$oRecord->id = "BanDomainModify";
$oRecord->resources[] = $oRecord->id;
$oRecord->title = ffTemplate::_get_word_by_code("mentions_ban_domain_modify_title");
$oRecord->src_table = CM_TABLE_PREFIX . "mod_mention_ban_domain";
$oRecord->addEvent("on_done_action", "BanDomainModify_on_done_action");
...
$oField = ffField::factory($cm->oPage);
$oField->id = "name";
$oField->label = ffTemplate::_get_word_by_code("mentions_ban_domain_modify_name");
...
function BanDomainModify_on_done_action($component, $action) {
	$db = ffDB_Sql::factory();
