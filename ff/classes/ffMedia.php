    ...
    public static function sendHeaders($file, $params = null) {
        ...
        if($file) {
          ...
          /*  if(!$size)              $size       = filesize($file);
            ...
            if(!$compress && $size) header("Content-Length: " . $size);*/
            if ($etag === true)     header("ETag: " . md5($file . filemtime($file)));
            elseif(strlen($etag))   header("ETag: " . $etag);
...
/*
    ...
    private function findSource($mode = null) {
        ...
        elseif(!$this->resolveSourcePath(ffMedia::UPLOAD_BASE_PATH, $mode)) {
            $this->resolveSourcePath(ffMedia::STATIC_BASE_PATH, $mode);
        }
    ...
    private function resolveSourcePath($base = ffMedia::UPLOAD_BASE_PATH, $mode = null) {
        if(!$this->filesource) {
            $image                                                  = $this->pathinfo;
            $basepath                                               = $base;
            ...
            */
            ...
            if(strpos($image["filename"], "-png-") !== false) {
                ...
                $res = $this->getModeByFile($base . $source["dirname"] . "/" . $image["filename"] . "." . $source["extension"]);
                if($res) {
