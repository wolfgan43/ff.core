 ...
 * @link http://www.formsphpframework.com
 */
class ffDBAdapter_db_sql_mysqli extends ffDB_Sql implements ffDBAdapter
{
	// CONSTRUCTOR
	...
	function queryStructure($eType, $sName = "", $lazy = true, $create_globals = false)
	{
		...
		switch ($eType)
		{
			case ffDBAdapter::TYPE_DB:
				if (false !== ($res = mysqli_query($this->link_id, "SHOW TABLES")))
				{
					$results = array();
			...
			case ffDBAdapter::TYPE_COLUMNS:
				if (false !== ($res = mysqli_query($this->link_id, "SHOW FULL COLUMNS FROM $sName")))
				{
					if (mysqli_num_rows($res))
					{
						$fields = Array();
			...
			case ffDBAdapter::TYPE_INDEXES:
				if (false !== ($res = mysqli_query($this->link_id, "SHOW INDEX FROM $sName")))
				{
					if (mysqli_num_rows($res))
					{
						$indexes = Array();
