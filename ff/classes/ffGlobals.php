...
class ffGlobals
{
    ...
    {
        if (!isset(ffGlobals::$instances[$bucket]))                 ffGlobals::$instances[$bucket] = new ffGlobals();

        return ffGlobals::$instances[$bucket];
    }

    public static function set($name, $value = null, $bucket = null) {
        if(!$name)                                                  {cms::getInstance("debug")->dump(); exit;}
    ...
    public static function del($name, $bucket = null) {
        if(isset(self::$instances[$bucket]->$name))                 unset(self::$instances[$bucket]->$name);

        return true;
    }
    public static function clear($bucket = null) {
        if(self::$instances[$bucket])                               unset(self::$instances[$bucket]);
