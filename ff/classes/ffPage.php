...
/**
 * ffPage è la classe che funge da contenitore e riferimento per tutti gli altri oggetti del framework,
...
/**
 * ffPage_base è la classe che funge da contenitore e riferimento per tutti gli altri oggetti del framework,
 * dai componenti agli elementi base.
 * E' indispensabile per realizzare una pagina che utilizzi gli automatismi del framework, per quanto sia possibile
 * utlizzare piccoli pezzetti del framework senza il suo intervento.
 *
...
abstract class ffPage_base extends ffCommon
{
	...
	/**
	 * Tema utilizzato da ffPage; di default è settato a "default"
	 * @var String
	 */
	var $theme 					= "default";
	...
	var $contents				= array();
	/**
	 * i gruppi in cui è suddivisa la pagina
	 * @var array()
	 */
	var $groups					= array();
	...
	/**
	 * Se il template principale è stato caricato o meno
	 * @var boolean
	 */
	...
	* 		Ogni contenuto deve avere un id univoco (componente->id) o ffPage sovrascriverà l'istanza.
	...
	*/
	public function addContent($content, $group = null, $id = null, $options = array())
	{
		...
		elseif ($group === true)
		{
			...
			$this->contents[(string)$id]["data"] = $content;
			$this->contents[(string)$id]["group"] = true;
			if (isset($options["title"]))
				$this->groups[$group]["title"] = $options["title"];
			if (isset($options["cols"]))
				$this->groups[$group]["cols"] = $options["cols"];
		}
		else
	...
	* 		Ogni componente deve avere un id univoco (componente->id) o ffPage sovrascriverà l'istanza.
	...
	*/
	 ...
	 * 		vengono passati e vanno gestiti, continuando per così dire a rimbalzare fra le pagine.
	...
	/**
	 * aggiunge alla pagina un campo nascosto. Funziona solo se è abilitato il form della pagina (abilitato per default).
	 * 
	 * @param string $sName il nome che dovrà assumere il campo nascosto
	 * @param object $oField una variabile contenente l'oggetto da utilizzare per estrarre il valore del campo.
	 * 							Può essere un ffData o un ffField. Nel secondo caso verrà usato il membro "value".
	 */
	 ...
	 * @param mixed $content il contenuto, può essere un oggetto od un contenuto testuale
	 * @return mixed
	 */
	...
	/**
	 * recupera i parametri dell'url con cui è stata chiamata la pagina sotto forma di query string
	...
	/**
	 * recupera i parametri con cui è stata chiamata la pagina sotto forma di query string
	...
	/**
	 * recupera le chiavi con cui è stata richiesta la pagina sotto forma di url
	 * @param array() $keys
	 * @return String
	 */
	...
	function widgetLoad($name, $path = null, &$ref = null)
	{
		...
		if (substr($realpath, 0, strlen(FF_DISK_PATH)) != FF_DISK_PATH)
	...
	function struct_process()
	{
		...
		{
			if ($this->getXHRComponent() && $this->getXHRComponent() !== $key)
				continue;

			$this->tpl[0]->set_var("objid", $key);
			$this->tpl[0]->set_var("type", substr(get_parent_class($value), 0, strpos(get_parent_class($value), "_")));

			$this->tpl[0]->set_var("SectResource", "");
			$this->tpl[0]->set_var("SectProperty", "");
			...
			if ($this->isXHR())
			{
				...
				$this->tpl[0]->parse("SectProperty",	true);
			}
			...
			if (is_array($this->components[$key]->resources) && count($this->components[$key]->resources))
			{
				...
				foreach ($this->components[$key]->resources as $subkey => $subvalue)
				{
					...
					if ($i > 1)
						...
						$this->tpl[0]->set_var("resource", '"' . $subvalue . '"');
					$this->tpl[0]->parse("SectResource", true);
				}
			...
			{
				case "ffGrid_base":
					$rc += $this->struct_process_fields($value->grid_fields, $key);
					$rc += $this->struct_process_fields($value->search_fields, $key, "_src");
					break;

				case "ffRecord_base":
					$rc += $this->struct_process_fields($value->form_fields, $key);
					break;
			...
			$this->tpl[0]->parse("SectFFObject", true);
		}
	 ...
	 */
	function struct_process_fields($fields, $component = null, $suffix = "")
	{
		if ($component !== null)
			$this->tpl[0]->set_var("getobj", 'get("' . $component . '").');
		else
			$this->tpl[0]->set_var("getobj", "");
		...
		if (is_array($fields) && count($fields))
		{
			foreach ($fields as $key => $field)
			{
				...
				$this->tpl[0]->set_var("SectFieldResource", "");
				if (is_array($field->resources) && count($field->resources))
				{
					...
					foreach ($field->resources as $subkey => $subvalue)
					{
						...
						if ($i > 1)
							...
							$this->tpl[0]->set_var("resource", '"' . $subvalue . '"');
						$this->tpl[0]->parse("SectFieldResource", true);
					}
