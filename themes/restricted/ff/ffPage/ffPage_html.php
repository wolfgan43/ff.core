		...
		*/	
		...
		if ($found)
		{
			...
			if (ffIsset($lib_data, "css_deps"))
			{
				...
				{
					$deps["css"][] = (strpos($css_key, ".") === 0 ? $tag : "") . $css_key;
					$ret = $this->tplAddCss(
							(strpos($css_key, ".") === 0 ? $tag : "") . $css_key
		...
		{
			$jquery_ui_theme = ($this->jquery_ui_force_theme ? $this->jquery_ui_force_theme : (
							$this->jquery_ui_theme ? $this->jquery_ui_theme : "base"
						));

			$this->tplAddCss(
				"jquery-ui.theme"
				, array(
					"file" => $jquery_ui_theme . "/theme.css"
...
//					$lib_data = $lib_value[$lib_version];
    ...
    {
		if (!(isset($this->page_meta[$type]) && isset($this->page_meta[$type][$name])) || $overwrite)
		{
			$this->page_meta[$type][$name] = $content;
			return true;
		}
		else
			return false;
	...
	protected function tplParse($output_result)
	{
		...
		if ($this->canonical)
		{
			$this->tpl[0]->set_var("canonical_href", $this->canonical);
			$this->tpl[0]->parse("SectCanonical", false);
		}
			...
			foreach ($this->contents as $key => $content)
			{
				...
				else
				{
					if (
							...
							&& (
								   is_subclass_of($content["data"], "ffGrid_base")
								|| is_subclass_of($content["data"], "ffRecord_base")
								|| is_subclass_of($content["data"], "ffDetails_base")
							)
						)
					{
						if ($content["data"]->use_own_location)
							continue;

						$tmp_found = false;
						foreach (cm::getInstance()->applets_components as $applet_id => $applet_comps)
						{
							if (ffIsset($applet_comps, $content["data"]->id))
							{
								$tmp_found = true;
								break;
							}
   ...
   /**
    ...
    {   
        ...
        {
            foreach ($this->page_meta as $key_type => $value_type)
            {
				foreach ($this->page_meta[$key_type] as $key_name => $value_name)
				{
					$this->tpl[0]->set_var("meta_type",		$key_type);
					$this->tpl[0]->set_var("meta_name",		$key_name);
					$this->tpl[0]->set_var("meta_content",	$value_name);
					$this->tpl[0]->parse("SectMeta", true);
				}
				reset($this->page_meta[$key_type]);
            }
