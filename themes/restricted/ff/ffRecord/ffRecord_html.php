...
class ffRecord_html extends ffRecord_base
{
	...
	protected function tplDisplayContents()
	{
		...
		foreach($this->contents as $key => $content)
		{
			...
			if (is_object($content["data"]) && is_subclass_of($content["data"], "ffField_base"))
			{
				...
				$this->tpl[0]->set_var("field_id", $content["data"]->id);
				...
				if (!$rc)
				{
					...
					if (is_array($this->form_fields[$content["data"]->id]->container_vars) && count($this->form_fields[$content["data"]->id]->container_vars))
					{
						foreach ($this->form_fields[$content["data"]->id]->container_vars as $field_key => $field_value)
						{
							$this->tpl[0]->set_var($field_key, $field_value);
							$vars_to_reset[$field_key] = true;
						}
						reset($this->form_fields[$content["data"]->id]->container_vars);
					}
...
/*				if ($this->form_fields[$content["data"]->id]->value->getValue())
				{
					$this->tpl[0]->parse("SectVal_" . $content["data"]->id, false);
					$this->tpl[0]->set_var("SectNotVal_" . $content["data"]->id, "");
				}
				else
				{
					$this->tpl[0]->set_var("SectVal_" . $content["data"]->id, "");
					$this->tpl[0]->parse("SectNotVal_" . $content["data"]->id, false);
				}
*/
			...
			elseif (is_string($content["data"]) || get_class($content["data"]) == "ffTemplate" || is_subclass_of($content["data"], "ffDetails_base") || is_subclass_of($content["data"], "ffGrid_base") || is_subclass_of($content["data"], "ffRecord_base"))
			{
				$this->tpl[0]->set_var("SectLabel", "");
				if (is_string($content["data"]))
					$this->tpl[0]->set_var("content", $content["data"]);
				else if (get_class($content["data"]) == "ffTemplate")
			...
			$this->tplSetFixedVars();
			$this->tpl[0]->set_var("field_n", $this->displayed_fields);
