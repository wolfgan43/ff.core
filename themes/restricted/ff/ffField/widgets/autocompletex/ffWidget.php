...
/*							, "ff.ffField.autocompletex"	=> array(
									...
									, "path" => "/themes/restricted/ff/ffField/widgets/autocompletex"
								)*/
						);
    var $css_deps 		= array(/*
	...
	function prepare_template($id)
	{
		...
		if ($this->innerURL !== false)
			$this->tpl[$id]->parse("SectInnerURL", false);
	}
	...
	function process($id, &$value, ffField_base &$Field)
	{
        ...
        if(strlen($Field->widget_path))
            ...
            $this->tpl[$tpl_id]->set_var("widget_path", "/themes/restricted/ff/ffField/widgets/autocompletex");
...
*/
        if($Field->autocomplete_combo) {
            $this->tpl[$tpl_id]->set_var("combo_class", cm_getClassByFrameworkCss("caret-down", "icon"));
...
/*        if(array_key_exists("autocomplete", $Field->parent_page[0]->widgets)) {
	...
	function get_component_headers($id)
	{
		if ($this->oPage !== NULL) { //code for ff.js
			...
			$this->oPage[0]->tplAddJs("ff.ffField.autocompletex", "autocompletex.js", FF_THEME_DIR . "/restricted/ff/ffField/widgets/autocompletex");
		}
