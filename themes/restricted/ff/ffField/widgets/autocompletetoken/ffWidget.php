...
//            $this->tpl[$tpl_id]->set_var("selected_id", $value->getValue($Field->get_app_type(), $Field->get_locale()));
		...
		if ($Field->autocompletetoken_service === null)
		{
			...
			if ($this->innerURL === null)
				$this->tpl[$tpl_id]->set_var("innerURL", $this->source_path . "/ff/ffField/widgets/autocompletetoken/parsedata." . FF_PHP_EXT . "?");
			else
				$this->tpl[$tpl_id]->set_var("innerURL", (strpos($this->innerURL, "?") === false ? $this->innerURL : substr($this->innerURL, 0, strpos($this->innerURL, "?"))));			
		}
		else
		...
		if (strlen($Field->source_SQL))
		{
            if($Field->autocompletetoken_service === null)
            {
			    ...
			    $ff = get_session("ff");
			    $ff["autocompletetoken"][$tmp]["sql"] 							= $Field->source_SQL;
			    ...
			    $ff["autocompletetoken"][$tmp]["operation"]						= $Field->autocompletetoken_operation;
			    $ff["autocompletetoken"][$tmp]["concat_field"]					= $Field->autocompletetoken_concat_field;
			    $ff["autocompletetoken"][$tmp]["concat_separator"]				= $Field->autocompletetoken_concat_separator;
