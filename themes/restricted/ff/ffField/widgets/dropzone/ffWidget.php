...
class ffWidget_dropzone extends ffCommon
{

    // ---------------------------------------------------------------
    //  PRIVATE VARS (used by code, don't touch or may be explode! :-)
    var $template_file 	 = "ffWidget.html";
    var $class			= "ffWidget_dropzone";
    var $widget_deps	= array();
    var $js_deps = array(
                        "jquery"                    => null
                        , "jquery.dropzone"         => "/plugins/jquery.dropzone"
                    );
    var $css_deps = array(
                        "dropzone" => array(
                                        "file"      => "dropzone.css"
                                        , "path"    => null
                                        , "rel"     => "plugins/jquery.dropzone"
                                    ) 
                    );
    ...
    {
        global $plgCfg_dropzone_UseOwnSession;
        //$Field->parent_page[0]->tplAddCss("jquery.dropzone", "dropzone.css", FF_SITE_PATH	 . "/themes/library/plugins/jquery.dropzone");
        ...
        if(strlen($Field->widget_path))
            ...
            $this->tpl[$tpl_id]->set_var("widget_path", "/themes/restricted/ff/ffField/widgets/dropzone");

		$css_deps["dropzone"]        = array(
                      "file" => "dropzone.css"
                    , "path" => null
                    , "rel" => "plugins/jquery.dropzone"
                );
		...
		if($Field->dropzone_showfile_plugin && is_file(FF_DISK_PATH . FF_THEME_DIR . "/library/plugins/jquery." . $Field->dropzone_showfile_plugin . "/jquery." . $Field->dropzone_showfile_plugin . ".js")) {
			if($Field->dropzone_showfile_plugin && is_file(FF_DISK_PATH . FF_THEME_DIR . "/library/plugins/jquery." . $Field->dropzone_showfile_plugin . "/jquery." . $Field->dropzone_showfile_plugin . ".css")) {
				$css_deps[$Field->dropzone_showfile_plugin] = array(
		                      "file" => "jquery." . $Field->dropzone_showfile_plugin . ".css"
		                    , "path" => null
		                    , "rel" => "plugins/jquery." . $Field->dropzone_showfile_plugin
		                );
		        $this->tpl[$tpl_id]->set_var("dropzone_plugin_name", $Field->dropzone_showfile_plugin);
		        $this->tpl[$tpl_id]->set_var("dropzone_plugin_css", FF_SITE_PATH . FF_THEME_DIR . "/library/plugins/jquery." . $Field->dropzone_showfile_plugin . "/jquery." . $Field->dropzone_showfile_plugin . ".css");
		        $this->tpl[$tpl_id]->parse("SectPluginCss", false);
			}
			$js_deps["jquery.fn." . $Field->dropzone_showfile_plugin] = array(
 						"file" => "jquery." . $Field->dropzone_showfile_plugin . ".js"
				        , "path" => FF_THEME_DIR . "/library/plugins/jquery." . $Field->dropzone_showfile_plugin
					);
			

	        $this->tpl[$tpl_id]->set_var("dropzone_plugin_name", "jquery." . $Field->dropzone_showfile_plugin);
	        $this->tpl[$tpl_id]->set_var("dropzone_plugin_js", FF_SITE_PATH . FF_THEME_DIR . "/library/plugins/jquery." . $Field->dropzone_showfile_plugin . "/jquery." . $Field->dropzone_showfile_plugin . ".js");
		...
		if($Field->dropzone_showfile_plugin) {
			$this->tpl[$tpl_id]->set_var("showfile_plugin", "'" . $Field->dropzone_showfile_plugin . "'");
		} else {
		...
		if(session_id() != '' && get_session("UserNID") != MOD_SEC_GUEST_USER_ID) { //if(session_status() == PHP_SESSION_NONE) {
			if ($plgCfg_dropzone_UseOwnSession || $Field->actex_use_own_session) 
				session_start();
			$ff = get_session("ff");
        ...
        if($Field->extended_type == "File") {
			...
			if(session_id() != '' && get_session("UserNID") != MOD_SEC_GUEST_USER_ID) {//if(session_status() == PHP_SESSION_NONE) {
				$ff["dropzone"][$tmp]["folder"] = $folder;
				$ff["dropzone"][$tmp]["base_path"] = $base_path;
		...
		if($Field->dropzone_showfile_path) {
			$this->tpl[$tpl_id]->set_var("showfile_path", "'" . $Field->dropzone_showfile_path . "'");
		} else {
			$this->tpl[$tpl_id]->set_var("showfile_path", "undefined");
		}
		
		$this->tpl[$tpl_id]->set_var("thumb_model", $Field->dropzone_model_thumb);
	...
	function get_component_headers($id)
	{
		if ($this->oPage !== NULL) { //code for ff.js
			$this->oPage[0]->tplAddJs("jquery.dropzone", "jquery.dropzone.js", FF_THEME_DIR . "/library/plugins/jquery.dropzone");
            $this->oPage[0]->tplAddJs("ff.ffField", "ffField.js", FF_THEME_DIR . "/library/ff");
			$this->oPage[0]->tplAddJs("ff.ffField.dropzone", "dropzone.js", FF_THEME_DIR . "/restricted/ff/ffField/widgets/dropzone");
		}
