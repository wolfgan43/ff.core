...
var PhotoWall = {
	...
	init: function(op) {	
	    PhotoWall.options = jQuery.extend(PhotoWall.options,op);

		PhotoWall._el = op.el+' .body';
		PhotoWall._c_width = jQuery(PhotoWall._el).width()-getScrollBarWidth();
		PhotoWall._c_height = jQuery(PhotoWall._el).height();	
		PhotoWall._line_max_height = PhotoWall.options.lineMaxHeight;
		PhotoWall._setLineMaxHeightDynamic();
		jQuery(PhotoWall._el).html('').addClass('clearfix');
		jQuery(window).resize(PhotoWall.RESIZE);
	},
    ...
    _setLineMaxHeightDynamic: function() {
        if(PhotoWall.options.lineMaxHeightDynamic) {
		    var fact = jQuery(window).height()/PhotoWall.options.baseScreenHeight;
		    PhotoWall.options.lineMaxHeight = (PhotoWall._line_max_height*fact >= 100)?(PhotoWall._line_max_height*fact):100;
		}  
    },
	RESIZE: function() {
		var w = jQuery(PhotoWall._el).width();
		var h = jQuery(PhotoWall._el).height();
    ...
    */
	show: function(data) {
        ...
        var addImage = function(id,padding,w,h,big,th,cw,ch) {
            ...
            var t = jQuery('<div id="'+id+'" class="pw-photo '+crop+' clearfix" style="'
                +'margin:'+padding+'px;'
			...
			if(ff.browser.msie) {
				t.find('img').hide().load(function(){jQuery(this).fadeIn(300);});
			} else {
				t.find('img').css('opacity',0).load(function(){
			        jQuery(this).delay(Math.random()*(1000 - 300)+300)
        ...
        */
		var showLine = function(line,total_width,last,first) {
		    var ln = jQuery("<div class='pw-line' style='width:"+(total_width+num_photos*PhotoWall.options.padding*2)+"'></div>")
	    ...
	    if(PhotoWall._c_width < jQuery(PhotoWall._el).width()) {
            PhotoWall.RESIZE();
	    }
		...
		if(PhotoWall.options.showBoxSocial) {
			...
			update  = function(){
				...
				if(typeof(twttr) !== 'undefined') {
					jQuery('#showbox .twitter-share-button').attr('data-url',location.href);
					twttr.widgets.load();
				}		            
			};
		}
	...
	*/
	initZoom: function() {
		$(document).on(PhotoWall.options.zoomAction, '.pw-zoom',function (e){
		...
		$(document).on("mouseleave", '.pw-zoom',function (e){
			var img = jQuery(e.target);
			var container  = img.parent().parent();
			var item   = PhotoWall._photos[container.attr('id')];
			if (PhotoWall._zoom_timeout) {
...
var ShowBox = {
    ...
    init: function(el,op) {
        ...
        ShowBox.options.push(jQuery.extend(a,op));
        ShowBox._init(el);
    ...
    _initEvents: function(el) {
            ...
            $(document).on("click", el,function (e){
                ...
                for(var i in ShowBox._images[gal]) {
                    if(ShowBox._images[gal][i][0] == src.attr('href')) {
                        ...
                        ShowBox._th = jQuery('#showbox-thc'+gal+' .showbox-th').eq(i).addClass('showbox-th-active');
                        break;
                    }
                }
                ShowBox._show(gal);
        ...
        if(!ShowBox._inited) {
            ...
            jQuery('#showbox-exit').click(function(){
                if(!ShowBox._opened) return;
                ShowBox.EXIT(this);
            });
            jQuery('#showbox .showbox-preview').mouseenter(function(){    
                if(ShowBox._preview_locked) return;
                ShowBox.OPENPREVIEW(this);
            });
            jQuery('#showbox .showbox-preview').mouseleave(function(){
                if(!ShowBox._opened) return;
                if(ShowBox._preview_locked) return;
                ShowBox.CLOSEPREVIEW(this);
            });
            jQuery('#showbox .showbox-preview').click(function(){
                if(!ShowBox._opened) return;
    ...
    _parseGet: function() {
        ...
        if(get['p'] && get['gal']) {
            ...
            ShowBox._index = p;
            ShowBox._current = gal;
            ShowBox._th = jQuery('#showbox-thc'+gal+' .showbox-th').eq(p).addClass('showbox-th-active');
            ShowBox._show(gal);
        }
    },
    _next: function() {
    ...
    _show: function(gal) {
        var thc = jQuery('#showbox-thc'+gal).detach();
        var mb = jQuery('#showbox-menubar'+gal).detach();
        ...
        jQuery('#showbox-loader').show();
        jQuery('body').css('overflow','hidden');
        jQuery('#showbox').fadeIn(200,function() {
            ShowBox._changeImage(ShowBox._index);
        });
    },
    _addThumb: function(gal,im,i) {
    	jQuery('<div class="showbox-th"><img src="'+im+'" /></div>')
    ...
    _changeImage: function(ind) {
        jQuery('#showbox-loader').show();
        jQuery('#showbox .showbox-menubar').hide();
        jQuery('#showbox .showbox-menubar').html('');	
        ind = parseInt(ind);
        var total = ShowBox._images[ShowBox._current].length;
        ShowBox._setCounter(ind+1,total);
        window.location.hash = 'p='+(ind+1)+'&gal='+(ShowBox._current+1);
        jQuery('#showbox .showbox-menubar').append('<div id="showbox-menubar'+ShowBox._current+'" style="position:relative;">'+ShowBox.options[ShowBox._current].menuBarContent+'</div>');
        ShowBox._onChangePhoto();
        ShowBox._index = ind;
        jQuery('#showbox .showbox-img').remove();
        ShowBox._th.removeClass('showbox-th-active');
        ShowBox._th = jQuery('#showbox .showbox-th').eq(ind).addClass('showbox-th-active');
        
        var img = jQuery('<img class="showbox-img" style="display:none;"/>')            
        ...
        .load(function(){
            ...
            jQuery('#showbox-loader').hide();   
            jQuery('#showbox .showbox-menubar').show();
            ShowBox.RESIZE();
        });
    },
    _setCounter: function(num,total) {
        jQuery('#showbox .showbox-th-counter').html(num+' of '+total);
    },
    _clearHash: function() {
    ...
    EXIT: function(el){
        ...
        jQuery('#showbox .showbox-menubar').hide();
        jQuery('#showbox .showbox-image img').remove();
		jQuery('#showbox-thc'+ShowBox._current).css({position:'absolute',top:'-10000px'}).detach().appendTo('body');
		jQuery('#showbox-menubar'+ShowBox._current).css({position:'absolute',top:'-10000px'}).detach().appendTo('body');
    },
    ...
    OPENPREVIEW: function(el){
        jQuery(el).animate({bottom:0},{queue: false,duration:150});
    },
    CLOSEPREVIEW: function(el){
        jQuery(el).animate({bottom:-75},{queue: false,duration:150});
    },
    LOCKPREVIEW: function(el){
        ShowBox._preview_locked = ShowBox._preview_locked?false:true;
        if(ShowBox._preview_locked) {
            jQuery('#showbox .showbox-pv-lock').show();
            jQuery(el).css({
                'background': '#111',
                'border-top': '1px solid #383838'
            });
        } else {
            jQuery('#showbox .showbox-pv-lock').hide();
            jQuery(el).css({
                'background': '',
                'border-top': ''
            });
    ...
    RESIZE: function(el){
        ...
        var img = jQuery('#showbox .showbox-image img');
        ...
        jQuery('#showbox .showbox-image').css({
            'width': imW+'px',
            'height': imH+'px',
            'margin-left': imL+'px',
            'margin-top': imT+'px'
        ...
        jQuery('#showbox .showbox-th-container').animate({
            left:  thL
