...
(function ($) {
	...
	function createBorder() {
		...
		else if(ff.browser.msie) { self.elements.tooltip.append('<v:image style="behavior:url(#default#VML);"></v:image>'); }
	...
	function positionTip(corner) {
		...
		ieAdjust = positionAdjust = (ff.browser.msie) ? 1 : 0;
		...
		if(corner.precedance === 'y') {
			...
			if(ff.browser.msie) {
				if(parseInt(ff.browser.version.charAt(0), 10) === 6) { positionAdjust = corner.y === 'top' ? -3 : 1; }
			...
			// Adjustments for IE6 - 0.5px border gap bug
			if(ff.browser.msie) {
				positionAdjust = (parseInt(ff.browser.version.charAt(0), 10) === 6) ? 1 : (corner.x === 'left' ? 1 : 2);
			}
		...
		if(ff.browser.msie && parseInt(ff.browser.version.charAt(0), 6) === 6) {
	...
	function createTip(corner) {
		...
		if(corner.y === 'top' && ff.browser.msie && parseInt(ff.browser.version.charAt(0), 10) === 6) {
			self.elements.tip.css({
				marginTop: -4
			});
		}
	...
	function construct() {
		...
		if(ff.browser.msie) { self.elements.wrapper.add(self.elements.content).css({ zoom: 1 }); }
		...
		if($('<canvas />').get(0).getContext || ff.browser.msie) {
			// Create border
	...
	function QTip(target, options, id) {
		...
		$.extend(self, self.options.api, {
			...
			updatePosition: function (event, animate) {
				...
				if(!self.elements.bgiframe && ff.browser.msie && parseInt(ff.browser.version.charAt(0), 10) === 6) {
					bgiframe.call(self);
				}
