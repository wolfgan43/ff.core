...
(function($) {
	...
	function tree_component () {
		return {
			...
			init : function(elem, conf) {
				...
				if(this.settings.ui.theme_name !== false) {
					if(this.settings.ui.theme_path === false) {
						...
						alert(_this.settings.ui.theme_path);
					}
			...
			attach_events : function () {
				...
				$(document).on("click",this.container.get(0),
				...
				$(document).on("dblclick", this.container.get(0),
					function (event) { // WHEN DOUBLECLICK ON TEXT OR ICON
						if(_this.locked) {
							event.preventDefault(); 
							event.stopPropagation();
							event.target.blur();
				...
				$(document).on("contextmenu", this.container.get(0),
					function (event) {
						if(_this.locked) {
							event.target.blur();
							return _this.error("LOCKED");
				...
				$(document).on("mouseover", this.container.get(0),
					function (event) {
						if(_this.locked) {
							event.preventDefault();
							event.stopPropagation();
				...
				$(document).on("mousedown", this.container.get(0),
					function (event) {
	...
	$(function () {
		...
		if(ff.browser.msie) { 
			if(ff.browser.version == 6) css += '.tree li { height:18px; zoom:1; } .tree li li { overflow:visible; } .tree .ltr li.last { margin-top: expression( (this.previousSibling && /open/.test(this.previousSibling.className) ) ? "-2px" : "0"); } .marker { width:45px; background-position:-32px top; } .marker_plus { width:5px; background-position:right top; }';
			if(ff.browser.version == 7) css += '.tree li li { overflow:visible; } .tree .ltr li.last { margin-top: expression( (this.previousSibling && /open/.test(this.previousSibling.className) ) ? "-2px" : "0"); }';
		}
		if(ff.browser.opera) css += '.tree > ul > li.last:after { content:"."; display: block; height:1px; clear:both; visibility:hidden; }';
		if(ff.browser.mozilla && ff.browser.version.indexOf("1.8") == 0) css += '.tree .ltr li a { display:inline; float:left; } .tree li ul { clear:both; }';
