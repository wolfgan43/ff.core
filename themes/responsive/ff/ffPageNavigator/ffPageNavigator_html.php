...
class ffPageNavigator_html extends ffPageNavigator_base
{
	...
	public function tplLoad()
	{
		...
		$this->tpl[0]->set_var("prefix", $this->prefix);
	...
	function process($output_result = FALSE)
	{
		...
		if($this->display_first)
        {
			...
			$this->tpl[0]->set_var("url", ffUpdateQueryString("page", false, $this->url));
		...
		if($this->display_prev) 
        {
			...
			$this->tpl[0]->set_var("url", ffUpdateQueryString("page", ($this->page == 1 ? $totpage : ($this->page) == 2 ? false :  $this->page - 1), $this->url));
		...
		if($this->display_next) 
        {
			...
			$this->tpl[0]->set_var("url", ffUpdateQueryString("page", ($this->page == $totpage ? false : $this->page + 1), $this->url));
		...
		if($this->display_last)
        {
			...
			$this->tpl[0]->set_var("url", ffUpdateQueryString("page", $totpage, $this->url));
		...
		if(!$this->infinite)
		{
	        ...
	        if($end_page > $start_page) {
				if(($end_page - $start_page)  > 20) {
					$lastNum = round(($end_page - $start_page) / 6);
				}
				...
				if($start != $start_page) {
					...
					{	
						...
						if($totpage >= $value) {
		                    ...
		                    $this->tpl[0]->set_var("url", ffUpdateQueryString("page", $value, $this->url));
	            ...
	            for ($i = $start; $i <= $end; $i++) {
					...
					$this->tpl[0]->set_var("url", ffUpdateQueryString("page", ($i > 1 ? $i : false), $this->url));
        ...
        $pagination_class["pages"] = Cms::getInstance("frameworkcss")->get("pages", "pagination");
        $this->tpl[0]->set_var("pagination_class", Cms::getInstance("frameworkcss")->getClass($this->framework_css["pagination"], $pagination_class));
