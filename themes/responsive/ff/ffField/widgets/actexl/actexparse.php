...
if ($actex_preserve_field && $selected_value !== "")
{
	...
	if (strpos($sSQL, "[PRES_WHERE]") && strpos($sSQL, "[PRES_OR]"))
	{
		$sSQL = str_replace("[PRES_OR]", "OR", $sSQL);
		$sSQL = str_replace("[PRES_WHERE]", $tmp, $sSQL);
	}
	else
	{
		$sSqlHaving .= $tmp;
...
if (is_array($_REQUEST["ffActex_parent_data"]) && count($_REQUEST["ffActex_parent_data"]))
{
	...
	{
		$sSQL = str_replace("[" . $key . "_PARENT_VALUE]", $db->toSql($value), $sSQL); 
	}
	reset($_REQUEST["ffActex_parent_data"]);
}

// remove unused
$sSQL = str_replace("[PRES_OR]", "", $sSQL);
$sSQL = str_replace("[PRES_WHERE]", "", $sSQL);
