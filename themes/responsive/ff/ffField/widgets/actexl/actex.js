...
function normalizeVal(value) {
	...
	if (typeof value !== "string")
		throw  "ff.ffField.actexl - Invalid value";
...
var activecombo = function(params) {
	...
	var innerURL			= params.innerURL;
	var theme_dir			= params.theme_dir;
	/* use class "ffActex_loader" on main element to remove it automatically */
	var loading_markup		= params.loading_markup || '<input class="form-control ffActex_loader" type="text" placeholder="Loading data..." readonly>';//'<div class="d-flex flex-fill justify-content-center"><div class="spinner-border spinner-border-sm text-primary" role="status"><span class="sr-only">Loading...</span></div></div>';
	...
	var that = {
		...
		, "getNode"  : function (jq) {
			if (jq) {
				return jQuery.fn.escapeGet("actex_" + __id);
			} else {
		...
		, "getFather" : function() {
			if (that.father)
				return ff.ffField.actexl.getInstance(that.father);
			else
				return undefined;
		}
		...
		, "getFatherValue" : function() {
			if (that.father)
				return ff.ffField.actexl.getInstance(that.father).value;
			else
				return null;
		}
		...
		, "change" : function (reset_childs, value) {
			...
			if (that.childs.length) {
				...
				that.childs.each(function (a, child) {
					rc &= ff.ffField.actexl.getInstance(child).refill(reset_childs ? (ff.ffField.actexl.getInstance(child).reset_value ? null : undefined) : undefined, undefined, undefined, reset_childs);
					//rc &= ff.ffField.actexl.getInstance(child).refill(reset_childs ? null : undefined, undefined, undefined, reset_childs);
				});
				return rc;
			} else {
				return true
		...
		, "resetChildsPreRedir" : function (value) {
			if (that.childs.length) {
				that.childs.each(function (a, child) {
					var tmp = ff.ffField.actexl.getInstance(child);
		...
		, "update" : function (new_value, reset_childs) {
			...
			if (that.childs.length) {
				that.childs.each(function (a, child) {
					child = ff.ffField.actexl.getInstance(child);
					if (!child.isFilled() || reset_childs || child.getOldFatherValue() !== new_value) {
						child.refill(reset_childs ? (child.options.reset_value ? null : undefined) : undefined, undefined, undefined, reset_childs);
						//child.refill(reset_childs ? null : undefined, undefined, undefined, reset_childs);
					} else {
		...
		, "refill" : function (new_value, father_value, force_refresh, reset_childs) {
			var node = that.getNode(true);
			new_value = normalizeVal(new_value);
			...
			if (that.options.data_src == "" && that.options.service === null) {
					...
					that.data.each(function (a, tmp_data){
						if ((father_value == null && that.father == null) || tmp_data[0] == father_value) {
							...
							if (
								...
								buffer += ' onChange="';
								buffer += 'ff.ffField.actexl.recalc(\'' + __id + '\',\'' + that.data.length + '\',\'' + separator + '\');" ';
				...
				if(that.options.hideEmpty && !that.data.length) {
					if(that.options.hideEmpty === true)
						...
						node.closest("." + that.options.hideEmpty).hide();
				}
				...
				that.update(new_value, reset_childs);

				that.doEvent({
					"event_name"	: "refill",
					"event_params"	: [node.get(0)]
				});
				
				ff.ffField.actexl.doEvent({
					"event_name"	: "refill",
				...
				delayed_request(str_data, new_value, father_value, force_refresh, reset_childs);
				return false;
			}
		}
		...
		, "child_error_display" : function () {
			...
			if (node) {
				...
				if (that.childs.length) {
					that.childs.each(function (a, child) {
						ff.ffField.actexl.getInstance(child).child_error_display();
					});
				}
			}
		}

		, "async_refill" : function (retData, selected_value, father_value, reset_childs) {
			var node = that.getNode(true);
			var buffer = "";
			...
			jQuery(".ffActex_loader", node).remove();
			...
			if(that.options.control_type == "input") {
				...
				for (var i = 0; i < retData.length; i++) {
					...
					if (
						...
						buffer += ' onChange="';
						buffer += 'ff.ffField.actexl.recalc(\'' + __id + '\',\'' + retData.length + '\',\'' + separator + '\');" ';
...
//							+ 'data-actex-idx="' + i + '" '
			...
			that.update(selected_value, reset_childs);
	...
	function delayed_request (str_data, selected_value, father_value, force_refresh, reset_childs) {
		ff.ajax.blockUI();
		...
		if(that.options.use_cache && !force_refresh) {
			...
			if (sources.isset(id_data_src) && sources.get(id_data_src).isset(father_value)) {
				if (sources.get(id_data_src).get(father_value) === true) {
					...
					controls_waiting.get(id_data_src).set(ff.getUniqueID(), {"id" : __id, "selected_value" : selected_value, "father_value" : father_value, "reset_childs" : reset_childs});
					return;
				}

				that.async_refill(sources.get(id_data_src).get(father_value), selected_value, undefined, reset_childs);
				return;
			}
		...
		var mydata = {
			...
			, "reset_childs" : reset_childs
			, "params" : {
				"targetComp" : !component || component === "" ? undefined : component
				, "targetField" : __id
			}
		};
	...
	function async_success (retData, textStatus) {
		...
		var reset_childs	= this.mydata.reset_childs
		var id_data_src = that.getCacheDataSrc();
		...
		var father_value = that.getFatherValue();
		if (retData.widget.actex["D" + id_data_src] === undefined) {
			console.log ("actex - async_success: service error");
		}
		retData = father_value ? 
				retData.widget.actex["D" + id_data_src]["F" + father_value] || []
				: retData.widget.actex["D" + id_data_src];

		that.async_refill(retData, selected_value, father_value, reset_childs);
		...
		if (that.options.use_cache) {
			...
			if (controls_waiting.isset(id_data_src)) controls_waiting.get(id_data_src).each(function (k, v) {
				var tmp = ff.ffField.actexl.getInstance(v.id);
				if (v.father_value === father_value) {
					tmp.async_refill(retData, v.selected_value, v.father_value, v.reset_childs);
	...
	function parent_enable (inst) {
		...
		if (inst.father) {
			...
			if (node && node.firstChild && node.firstChild.disabled)
			{
				...
				var res = ff.ffField.actexl.doEvent({
					"event_name"	: "enable",
					"event_params"	: [father]
				});
			}
	...
	function cascade_disable (moveup, inst) {
		...
		if (moveup && inst.father) {
			...
			for (var a = 0; a < inst.childs.length; a++) {
				cascade_disable(false, ff.ffField.actexl.getInstance(inst.childs[a]));
			}
		}
	};
	...
	function display_loading () {
		...
		jQuery.fn.escapeGet(__id, node).remove();
		
		node.prepend(that.loading_markup || loading_markup);
		if(!that.options.hideEmpty || that.options.hideEmpty === true)
			node.show();
		else 
			node.closest("." + that.options.hideEmpty).show();
	...
	//jQuery("#calendar-modify_ID_customer_label").data("ui-autocomplete")._trigger("change");	
		jQuery.fn.escapeGet(__id + "_label").autocomplete({
			...
			if (event.relatedTarget) {
				...
				if (ff.ffField.actexl.exists(related)) 
					ff.ffField.actexl.getInstance(related).has_focus = true; // verificare se dopo il passaggio a blur ha ancora senso
			}
			...
			/*if (!change_rc && ff.ffField.actexl.clickTarget) {
				/*var tmp = ff.ffField.actexl.getInstance(ff.ffField.actexl.clickTarget.id);
				if (tmp) {
					tmp.addEvent({
						"event_name" : "refill",
						"func_name" : function (id, node) {
							console.log(id, node, jQuery("#" + ff.ffField.actexl.clickTarget.id));
						}
					});
				} else {
					jQuery.fn.escapeGet("#" + ff.ffField.actexl.clickTarget.id).click();
				}*/
			//}
		});
	...
	function drawDialogButtons () {
		if (ff.ffPage.dialog.dialog_params.get("actex_dlg_" + __id)) {
			if (!buttons.get("add") || that.options.limit_select) {
				jQuery.fn.escapeGet("actex_" + __id + "_dialogaddlink").addClass("disabled");
			} else {
				jQuery.fn.escapeGet("actex_" + __id + "_dialogaddlink").removeClass("disabled");
			}
		}

		if (ff.ffPage.dialog.dialog_params.get("actex_dlg_edit_" + __id)) {
			if (!buttons.get("edit")) {
				jQuery.fn.escapeGet("actex_" + __id + "_dialogeditlink").addClass("disabled");
				...
				jQuery.fn.escapeGet("actex_" + __id + "_dialogeditlink").removeClass("disabled");
			}
		}

		if (ff.ffPage.dialog.dialog_params.get("actex_dlg_delete_" + __id)) {
			if (!buttons.get("delete")) {
				jQuery.fn.escapeGet("actex_" + __id + "_dialogdeletelink").addClass("disabled");
			} else if (that.options.select_one && that.options.select_one_val == ff.coalesce(that.value, "")) {
				jQuery.fn.escapeGet("actex_" + __id + "_dialogdeletelink").addClass("disabled");
			} else if (that.options.select_noone && that.options.select_noone_val == ff.coalesce(that.value, "")) {
				jQuery.fn.escapeGet("actex_" + __id + "_dialogdeletelink").addClass("disabled");
			} else {
				jQuery.fn.escapeGet("actex_" + __id + "_dialogdeletelink").removeClass("disabled");
			}
		}
	};
	...
	function parseheader (value) {
		...
		if (that.options.control_type == "input") { 
			...
			buffer += ' onChange="ff.ffField.actexl.change(\'' + __id +'\', ' + that.options.reset_childs + ');" >';
...
"getInstance" : function (id, avoid_undef) {
	...
	if (tmp === undefined && !avoid_undef)
		throw "ff.ffField.actexl - instance does not exists [" + id + "]";
...
"refill" : function (id, selected_value, father_value, force_refresh, reset_childs) {
	that.getInstance(id).refill(selected_value, father_value, force_refresh, reset_childs);
},
...
"onClearField" : function (component, field_id, field_data, inst_id) {
	...
	if (that.exists(inst_id)) {
		ff.ffField.actexl.deleteCombo(inst_id);
	}
},
		...
		switch (ff.struct.get("comps").get(component).type) {
			case "ffDetails":
				...
				for (var r = 0; r < rows; r++) {
					...
					if (that.exists(tmp_id)) ajaxUpdate(tmp_id, field);
				}
				break;
...
function ajaxUpdate (id, field) {
	...
	for (var i = 0; i < field.resources.length; i++) {
		if (chainupdate.resources.isset(field.resources[i])) {
			inst.refill((inst.insert_mode ? chainupdate.resources.get(field.resources[i]) : undefined), undefined, true);
		}
	}
...
/*jQuery(document).mousedown(function (event) {
	ff.ffField.actexl.clickTarget = event.target;
	var parent = null;
	if (event.target && (parent = jQuery(event.target).parents(".ui-button")) && parent.length) {
		ff.ffField.actexl.clickTarget = parent.get(0);
	}
});

jQuery(document).mouseup(function (event) {
	ff.ffField.actexl.clickTarget = null;
});*/
